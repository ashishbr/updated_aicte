...
<input type="usn" placeholder="usn ID" name="usn" value="<?php echo $usn; ?>" required >
...
if(isset($_POST["update"])){
    ...
    $usn1 =isset($_POST['usn']) ? $_POST['usn'] : '';
    $phone_number1 = isset($_POST['phone_number']) ? $_POST['phone_number'] : '';
    if($usn == $usn1 && $phone== $phone_number1){
        $sql_update = "UPDATE `user_info`.`user` set `name`= '$name1' ,`usertype`='$user type1'   where `id` = '$user.id' ";
    ...
    $sql_usn="SELECT * FROM `user` WHERE `usn`= '$usn1'";
    ...
    if( $usn!= $usn1 && mysqli_num_rows($db_usn)!=0){
    ...
    $sql_update = "UPDATE `user_info`.`user` set `name`= '$name1' , `usn`= '$usn1', `phone_number` = '$phone_number1' where `id` = '$id' ";
    $result = mysqli_query($data, $sql_update);
    if($result)
  {
    echo "<script>alert('User information updated successfully!');</script>";
